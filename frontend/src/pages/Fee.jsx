    ...
    if (filters.length || search) {
      ...
      if (filters.includes("HET_HAN_TRONG_1_TUAN")) {
        searchFee = searchFee.filter(
          (f) =>
            getDaysBetweenDates(f.deadline, today) <= 7 &&
            getDaysBetweenDates(f.deadline, today) >= 0
        );
      }
  ...
  return (
    <div className="w-full h-screen relative p-2 px-8">
      ...
      <form
        ...
        } absolute right-1/2 translate-x-1/2 top-32  z-10 flex gap-16 px-8 py-8 bg-white max-w-[700px] items-start shadow-lg transition-all`}
      >
          ...
          <div className="flex mt-4 gap-12 items-start justify-start w-full max-h-[300px]">
            ...
            {feeType === "HOA_DON" ? (
              ...
              <input
                type="number"
                value={costBAT_BUOC}
                onChange={(e) => setCostBAT_BUOC(e.target.value)}
                className="text-red-400 min-w-[300px] border-b-2 outline-none p-1 px-2  focus:border-secondary transition-all"
                placeholder="ex: 120"
                required
              />
            ) : (
            ...
            >
                ...
                {fee.feeType === "TU_NGUYEN" ? (
                  <>
                    ...
                    <span className="text-violet-400">
                      {" "}
                      |{" "}
                      {fee.feepayInfo.reduce(
                        (accumulator, info) => accumulator + info.payed,
                        0
                      )}{" "}
                    </span>
                  </>
              ...
              <div className="text-center text-gray-400">
                ...
                <FontAwesomeIcon
                  ...
                  className="ml-6 invisible group-hover:visible hover:opacity-80 hover:scale-110  transition-all"
                />
              </div>
            </div>
          ))}
